...
class SyncLogsService(CommonService):
    ...
    def duplicate_and_parse(cls, kb, docs, tenant_id, src, auto_parse=True):
        ...
        if external_id_map:
            # Query existing documents by source_type and location
            existing_docs = DocumentService.query(kb_id=kb.id, source_type=src)
            for doc in existing_docs:
                # Store by location for quick lookup
                if doc.location:
                    existing_docs_by_location[doc.location] = doc
        
        # Separate docs into new and updated
        files_to_create = []
        docs_to_update = []
        ...
        for d in docs_to_upload:
            ...
            # Check if document exists by looking for matching location pattern
            # The location pattern for external sources is typically just the filename
            existing_doc = None
            for loc, doc in existing_docs_by_location.items():
                # Match by filename in location (handles different path separators)
        ...
        for existing_doc, doc_data, filename in docs_to_update:
            try:
                ...
                if auto_parse and auto_parse != "0":
                    # Remove old chunks before re-parsing
